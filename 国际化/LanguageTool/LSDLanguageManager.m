...
@implementation LSDLanguageManager

+(instancetype)shareManager{
    
    static LSDLanguageManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[LSDLanguageManager alloc]init];
    });
    return manager;
}
...
- (NSString *)getStringForKey:(NSString *)key tbl:(NSString *)tbl{
    ...
    if (!tbl) {
        tbl = NSLocalizedStringTableName;
    }
    
    if (key.length > 0) {
        if (_bundle) {
            NSString *str = NSLocalizedStringFromTableInBundle(key, tbl, _bundle, nil);
            if (str.length > 0) {
                return str;
            }
        }
